...
type metadata = {
  expr_loc : Location.t;
  type_reference : Types.t TypeTree.t;
  level : int;
}
...
module Annotations = struct
  ...
  let gen_fresh_t () =
    ...
    Types.TPoly(Core.sprintf "T%i" tv)
  ...
  let rec typeof_value = function
    ...
    | Int _    -> Types.TInt
    | Array _  -> Types.TArray (gen_fresh_t ())
    | String _ -> Types.TString
    | Symbol _ -> Types.TSymbol
    | Nil      -> Types.TNil
    | Any      -> gen_fresh_t ()
end
...
module Constraint_engine = struct
  ...
  let rec typeof_expr = function
    ...
    | ExprIVar ((_, value))
    | ExprConst ((_, value), _)
    | ExprValue (value) -> type_tree_node @@ Annotations.typeof_value value
  ...
  let rec build_constraints constraint_map (expr, { type_reference; level }) =
    let build_constraint type_key = function
      | ExprValue(v) ->
        TypeTree.union (type_tree_node @@ Annotations.typeof_value v) type_reference;
        constraint_map
        |> append_constraint type_key (Literal (type_tree_node @@ Annotations.typeof_value v))
